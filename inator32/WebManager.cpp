...
WebManager::WebManager(int port) :
#ifdef ESP8266
...
bool WebManager::isAuthenticated(String fromWhere) {
  if (hasHeader("Cookie")) {
  ...
  if (fromWhere != "") {
    fromWhere = "/login?location=" + fromWhere;
  } else {
    fromWhere = "/login";
  }
  sendHeader("Location", fromWhere);
...
void WebManager::handleLoginGet()
{
  ...
  repvars *r = templ.newRepvar(String("@location@"), String(loc.c_str()));
  templ.generateOutput(&server, f, r);
  f.close();
}
...
void WebManager::handleLoginPost()
{
  ...
  if (server.hasArg("user") && server.hasArg("pass") &&
    ...
    String target = server.arg("location");
    if (target == "") { target = "/"; }
...
{
  if (!server.isAuthenticated("/status")) {
    return;
  }
  ...
  r = templ.addRepvar(r, String("@ID@"), String(getChipId()));
...
#ifdef ESP32
  r = templ.addRepvar(r, String("@RealFlashSize@"), String("unknown on ESP32"));
#else
  r = templ.addRepvar(r, String("@RealFlashSize@"), String(ESP.getFlashChipRealSize()));
#endif
  r = templ.addRepvar(r, String("@CompilerFlashSize@"), String(ESP.getFlashChipSize()));
  r = templ.addRepvar(r, String("@FlashChipSpeed@"), String(ESP.getFlashChipSpeed()));
  r = templ.addRepvar(r, String("@FlashChipMode@"), String(ESP.getFlashChipMode()));
  r = templ.addRepvar(r, String("@WifiPower@"), String(wifi.getWifiPower()));
...
{
  if (!server.isAuthenticated("/config")) {
    return;
  }
...
{
  // The form submission is lost no matter what; send them back to
  // /config after logging back in
  if (!server.isAuthenticated("/config")) {
    return;
  }
  ...
  myprefs.set("hashmat", new_hashmat);
...
{
  if (!server.isAuthenticated("/restart")) {
    return;
  }
