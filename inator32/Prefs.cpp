...
void Prefs::setDefaults()
{
  ...
  strncat(mdnsName, String(getChipId()).c_str(), sizeof(mdnsName)-strlen(mdnsName));
  ...
  set("hashmat", "defaultAppPw"); // also sets cookie key
}
...
void Prefs::set(const char *what, const char *newVal)
{
  if (!strcmp(what, "ssid")) {
    ...
    cookieKey[1] = CRC32::calculate((uint8_t *)(&cookieKey[0]), 4);
    cookieKey[2] = CRC32::calculate((uint8_t *)(&cookieKey[1]), 4);
    cookieKey[3] = CRC32::calculate((uint8_t *)(&cookieKey[2]), 4);
  }
}
