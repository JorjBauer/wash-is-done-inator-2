...
void WifiManager::loop()
{
  ...
  if ((millis() > nextWifiScan) &&
      ((softAP && myprefs->ssid[0]) ||
       (!softAP && myprefs->ssid[0] && (WiFi.status() != WL_CONNECTED)))
      ) {
...
void WifiManager::StartSoftAP()
{
  ...
  esp_wifi_set_country_code("US", true); // FIXME needs abstracting
  String networkName = baseName + String("-") + getChipId();
...
int8_t WifiManager::getWifiPower()
{
  int8_t pwr = 0;
